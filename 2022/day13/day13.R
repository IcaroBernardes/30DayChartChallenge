...
func <- function(name, rank, url) {
  ...
  magick::image_write(img, glue::glue("2022/day13/images/{name}.png"))
}
purrr::pwalk(breed_rank_all, ~func(..1, ..2, ..3))

## Keeps the path to the local images
breed_rank_all <- breed_rank_all %>%
  dplyr::mutate(image = glue::glue("2022/day13/images/{joiner}.png")) %>%
  dplyr::select(joiner, `2020 Rank`, image)
...
## Saves the plot
ggsave("2022/day13/day13.png", plot = p, dpi = "retina", bg = bgcolor,
